...
void initDefaultPrograms() {
    ...
    defaultFragmentPrograms["landscape"] = ""
        ...
        "	float xscale = texScale * repeat / (2.0 * pi);\n"
        "	float yscale = texScale * 2.0 / (2.0 * pi);\n"
        "	float x = relv.x / (relv.z * zoom) + atan(facev.x, facev.y);\n"
        "	float y = relv.y / (relv.z * zoom) - (facev.z * pitch_adjust);\n"
        "	vec4 color = texture2D(texture0, vec2(sign(xscale) * (1.0 - 0.25 * repeat - x * xscale), offset - y * yscale));\n"
