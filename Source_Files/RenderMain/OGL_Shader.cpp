...
void initDefaultPrograms() {
    ...
    defaultFragmentPrograms["invincible"] = ""
        ...
        "   float mtime=mod(time,60.0)+1.0;\n"
        "   float eX=mtime*round(gl_FragCoord.x / blockSize);\n"
        "   float eY=mtime*round(gl_FragCoord.y / blockSize);\n"
    ...
    defaultFragmentPrograms["invincible_bloom"] = ""
        ...
        "   float blockWidth=100.0;\n"
        "   float darkBlockProbability=0.6;\n"
        "   float mtime=mod(time,60.0)+1.0;\n"
        "   float eX=mtime*round((gl_FragCoord.x + mtime*10.0) / blockWidth);\n"
        "   float eY=mtime*round((gl_FragCoord.y + mtime*10.0) / blockHeight);\n"
        "   vec2 entropy = vec2 (eX, eY); \n"
        "   float sr = rand(entropy); \n"
        "   float sg = rand(entropy*sr); \n"
        "   float sb = rand(entropy*sg); \n"
        ...
        "#endif\n"
        "   float fogFactor = clamp(exp2(FDxLOG2E * length(viewDir)), 0.0, 1.0);\n"
        "   gl_FragColor = vec4(mix(gl_Fog.color.rgb, intensity, fogFactor), vertexColor.a * color.a * 0.8);\n"
        "}\n";
