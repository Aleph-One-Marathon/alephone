...
void initDefaultPrograms() {
    ...
    defaultFragmentPrograms["invincible"] = ""
        ...
        "void main(void) {\n"
        "   float blockSize=6.0;"
        "   vec2 entropy = vec2 (time*round(gl_FragCoord.x / blockSize),time*round(gl_FragCoord.y / blockSize)); \n"
    ...
    defaultFragmentPrograms["invincible_bloom"] = ""
        ...
        "   vec2 entropy = vec2 (time*round((gl_FragCoord.x+time*10.0) / (blockSize*50.0)),time*round((gl_FragCoord.y+time*10.0) / blockSize)); \n"
        "   float sr = rand(entropy); \n"
        "   float sg = rand(entropy*sr); \n"
        "   float sb = rand(entropy*sg); \n"
