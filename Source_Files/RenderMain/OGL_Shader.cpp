...
#include "Shaders/sprite_infravision.frag"
    ...
    defaultFragmentPrograms["invincible_bloom"] = ""
        ...
        "   float darkThreshold=0.97;\n"
        "   float moment=fract(time/10000.0);\n"
        "   float eX=seed*round((gl_FragCoord.x) / blockWidth);\n"
        "   float eY=seed*round((gl_FragCoord.y+time) / blockHeight);\n"
        "   vec2 entropy = vec2 (eX, eY); \n"
        "   float sr = rand(entropy); \n"
        "   float sg = rand(entropy*sr); \n"
        "   float sb = rand(entropy*sg); \n"
        "   float flickerEX=moment*round((gl_FragCoord.x + mod(time, 60.0)*7.0) / blockWidth);\n"
        "   float flickerEY=moment*round((gl_FragCoord.y+time + mod(time, 60.0)*11.0) / blockHeight);\n"
        "   vec2  flickerEntropy = vec2 (flickerEX, flickerEY); \n"
        "   float flicker = 0.6 + 0.4 * rand(flickerEntropy); \n"
        "   vec3 intensity = vec3(0.0,0.0,0.0);\n"
        "   if (sr > darkThreshold || sg > darkThreshold || sb > darkThreshold) {\n"
        "      intensity = vec3(sr*sr, sg*sg, sb) * flicker; }\n"
        "   vec4 color = texture2D(texture0, gl_TexCoord[0].xy);\n"
        "   float dropFactor = max(max(intensity.r, intensity.g), intensity.b);\n"
