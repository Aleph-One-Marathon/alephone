...
void initDefaultPrograms() {
    ...
    defaultFragmentPrograms["blur"] = ""
        ...
        "const float f0 = 0.4930;\n"
        "const float f1 = 0.2297;\n"
        "const float f2 = 0.0232;\n"
        "const float f3 = 0.0005;\n"
        "#else\n"
        "const float f0 = 0.1468;\n"
        "const float f1 = 0.2506;\n"
        "const float f2 = 0.1332;\n"
        "const float f3 = 0.0427;\n"
        "#endif\n"
        "void main (void) {\n"
        "	vec2 s = vec2(offsetx, offsety);\n"
        "	// Thanks to Renaud Bedard - http://theinstructionlimit.com/?p=43\n"
        "	vec3 t = f0 * texture2DRect(texture0, gl_TexCoord[0].xy).rgb;\n"
        "	t += f1 * texture2DRect(texture0, gl_TexCoord[0].xy - 1.45*s).rgb;\n"
        "	t += f1 * texture2DRect(texture0, gl_TexCoord[0].xy + 1.45*s).rgb;\n"
        "	t += f2 * texture2DRect(texture0, gl_TexCoord[0].xy - 3.39*s).rgb;\n"
        "	t += f2 * texture2DRect(texture0, gl_TexCoord[0].xy + 3.39*s).rgb;\n"
        "	t += f3 * texture2DRect(texture0, gl_TexCoord[0].xy - 5.33*s).rgb;\n"
        "	t += f3 * texture2DRect(texture0, gl_TexCoord[0].xy + 5.33*s).rgb;\n"
