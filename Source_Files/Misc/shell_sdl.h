...
#define LIST_SEP ':'
  ...
  {
    vector < DirectorySpecifier >::const_iterator i =
      data_search_path.begin (), end = data_search_path.end ();
    while (i != end)
      {
...
static void
initialize_marathon_music_handler (void)
{
  FileSpecifier file;
...
alert_user (short severity, short resid, short item, OSErr error)
{
  ...
  if (SDL_GetVideoSurface () == NULL)
  ...
  else
    {
      ...
      dialog d;
      d.add (new
	     w_static_text (severity == infoError ? "WARNING" : "ERROR",
      ...
      while (strlen (t))
	{
      ...
      d.add (new w_spacer ());
      d.add (new
	     w_button (severity == infoError ? "OK" : "QUIT", dialog_ok, &d));
      d.run ();
...
{
public:
  w_levels (const vector < entry_point > &items,
	    dialog * d):w_list < entry_point > (items, 400, 8, 0), parent (d)
  {
  }
  ...
  void draw_item (vector < entry_point >::const_iterator i, SDL_Surface * s,
		  int x, int y, int width, bool selected) const
  {
    y += font->get_ascent ();
    char str[256];
      ...
      draw_text (s, str, x, y,
		 selected ? get_dialog_color (ITEM_ACTIVE_COLOR) :
...
short
get_level_number_from_user (void)
{
  // Get levels
  vector < entry_point > levels;
  if (!get_entry_points
      (levels,
       _single_player_entry_point | _multiplayer_carnage_entry_point |
  ...
  dialog d;
  d.add (new
	 w_static_text ("Before proceeding any further, you", MESSAGE_FONT,
			TITLE_COLOR));
  d.add (new
	 w_static_text ("must take the oath of the vidmaster:", MESSAGE_FONT,
			TITLE_COLOR));
  d.add (new w_spacer ());
  ...
  d.add (new
	 w_static_text ("and to never, ever, leave a single Bob alive.\xd3"));
  d.add (new w_spacer ());
...
{
public:
  w_file_list (const vector < dir_entry > &items):w_list < dir_entry > (items,
									400,
									15, 0)
  {
  }

  void draw_item (vector < dir_entry >::const_iterator i, SDL_Surface * s,
		  int x, int y, int width, bool selected) const
  {
    y += font->get_ascent ();
    set_drawing_clip_rectangle (0, x, s->h, x + width);
    draw_text (s, i->name.c_str (), x, y,
	       selected ? get_dialog_color (ITEM_ACTIVE_COLOR) :
	       get_dialog_color (ITEM_COLOR), font, style);
    set_drawing_clip_rectangle (SHRT_MIN, SHRT_MIN, SHRT_MAX, SHRT_MAX);
  }
...
bool
FileSpecifier::ReadDialog (int type, char *prompt)
{
  // Set default prompt
  if (prompt == NULL)
    {
...
{
public:
  w_file_name (const char *name, dialog * d, const char *initial_name =
	       NULL):w_text_entry (name, 31, initial_name), parent (d)
  {
  }
   ~w_file_name ()
  {
...
public:
  w_write_file_list (const vector < dir_entry > &items, const char *selection,
		     dialog * d, w_file_name * w):w_file_list (items),
    parent (d), name_widget (w)
  {
    if (selection)
      {
...
bool
FileSpecifier::WriteDialog (int type, char *prompt, char *default_name)
{
  // Set default prompt
  if (prompt == NULL)
    {
  ...
  w_write_file_list *list_w =
    new w_write_file_list (entries, default_name, &d, name_w);
  d.add (list_w);
  d.add (new w_spacer ());
  d.add (name_w);
  d.add (new w_spacer ());
...
bool
FileSpecifier::WriteDialogAsync (int type, char *prompt, char *default_name)
{
  return FileSpecifier::WriteDialog (type, prompt, default_name);
}

static bool
confirm_save_choice (FileSpecifier & file)
{
...
static void
process_screen_click (const SDL_Event & event)
{
...
static void
handle_game_key (const SDL_Event & event)
{
  ...
  switch (key)
    ...
    case SDLK_PERIOD:		// Sound volume up
      changed_prefs =
	adjust_sound_volume_up (sound_preferences, _snd_adjust_volume);
      break;
    case SDLK_COMMA:		// Sound volume down
      changed_prefs =
	adjust_sound_volume_down (sound_preferences, _snd_adjust_volume);
      break;
...
static void
process_game_key (const SDL_Event & event)
{
  switch (get_game_state ())
    {
    case _game_in_progress:
...
static void
process_system_event (const SDL_Event & event)
{
...
dump_screen (void)
{
  ...
  SDL_Surface *t =
    SDL_CreateRGBSurface (SDL_SWSURFACE, video->w, video->h, 24,
  ...
  for (int y = 0; y < video->h; y++)
    memcpy ((uint8 *) t->pixels + t->pitch * y,
	    (uint8 *) pixels + video->w * 3 * (video->h - y - 1),
	    video->w * 3);
  free (pixels);
