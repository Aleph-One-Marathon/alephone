...
static void crosshair_dialog(void *arg)
{
	...
	placer->dual_add(new w_static_text("クロスヘアーを表\示"), d);
	placer->add(new w_spacer, true);
...
static void online_dialog(void *arg)
{
	...
	lobby_table->dual_add(mute_guests_w->label("全てのゲストチャットを表\示ししない"), d);
...
{
	if (w->get_selection()) {
		mouse_up_w->set_text("上を向く");
		mouse_down_w->set_text("下を向く");
	} else {
		mouse_up_w->set_text("上を向く");
		mouse_down_w->set_text("下を向く");
	}
...
static void controls_dialog(void *arg)
{
	...
	std::vector<std::string> labels = { "照準", "移動", "行動", "インターフェース", "その他" };
	w_tab *tab_w = new w_tab(labels, tabs);
	...
	std::vector<std::string> mouse_aiming_labels = { "オフ", "生の入力", "OSの入力" };
	...
	look_options->dual_add(mouse_aiming_w->label("マウスの照準"), d);
	...
	look_options->dual_add(sens_horizontal_w->label("マウスの感度"), d);
	...
	look_options->dual_add(invert_mouse_w->label("マウスの動きを反転"), d);
	...
	look_options->dual_add(accel_mouse_w->label("マウスの加速"), d);
	look_options->dual_add(accel_mouse_w, d);
	
	look_options->add_row(new w_spacer(), true);
	
	std::vector<std::string> joystick_aiming_labels = { "ボタンとして使う", "アナログスティックとして使う" };
	...
	look_options->dual_add(joystick_aiming_w->label("コントローラーの照準"), d);
	...
	look_options->dual_add(sens_joy_w->label("コントローラーの感度"), d);
...
static void environment_dialog(void *arg)
{
	...
	w_env_select* resources_w = new w_env_select(environment_preferences->resources_file, "利用可能\なファイル", _typecode_unknown, &d);
	table->dual_add(resources_w->label("外部リソ\ース"), d);
