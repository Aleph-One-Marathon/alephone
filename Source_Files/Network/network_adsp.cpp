	...
	long buffer_size)
{
	...
	if (error==noErr)
	{
		/* the request is queued and weÕll be notified, with luck, when our data arrives */
	}
...
thereÕs lots of parameter voodoo here.  this routine catches, processes, and forwards the ring
packet.
*/
	...
	DSPPBPtr thePBPtr)
{
	...
	if (thePBPtr->ioResult==noErr)
	{
		...
		/* if weÕre not the server (player index zero), forward this packet immediately.  if we
			are the server then weÕll only forward this packet when our time manager task fires */
...
returns the ioResult field of the connectionEndÕs parameter block.
*/
